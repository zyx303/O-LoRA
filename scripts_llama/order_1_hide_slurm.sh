...
#SBATCH --output=order_1_hide-%j.out
#SBATCH --error=order_1_hide-%j.err
set -x
echo "order1_hide"
...
CHECKPOINT_FILE="logs_and_outputs_llama/hide/order_1/.checkpoint"
mkdir -p logs_and_outputs_llama/hide/order_1
...
# Task 1: dbpedia
if ! is_task_completed "1-dbpedia" "logs_and_outputs_llama/hide/order_1/outputs/1-dbpedia"; then
...
# Task 2: amazon
if ! is_task_completed "2-amazon" "logs_and_outputs_llama/hide/order_1/outputs/2-amazon"; then
...
# Task 3: yahoo
if ! is_task_completed "3-yahoo" "logs_and_outputs_llama/hide/order_1/outputs/3-yahoo"; then
    echo "Starting Task 3: yahoo"
    deepspeed --master_port $port src/run_uie_hide.py \
   --do_train \
   --do_predict \
   --predict_with_generate \
   ...
   --gradient_accumulation_steps 8 \
    ...
    --dataloader_num_workers 4 \
    --remove_unused_columns False
    mark_task_completed "3-yahoo"
    sleep 5
else
    echo "Skipping Task 3: yahoo (already completed)"
fi

# Task 4: agnews
if ! is_task_completed "4-agnews" "logs_and_outputs_llama/hide/order_1/outputs/4-agnews"; then
    echo "Starting Task 4: agnews"
    deepspeed --master_port $port src/run_uie_hide.py \
   --do_train \
   --do_predict \
   --predict_with_generate \
   ...
   --gradient_accumulation_steps 8 \
    ...
    --dataloader_num_workers 4 \
    --remove_unused_columns False
    mark_task_completed "4-agnews"
else
    echo "Skipping Task 4: agnews (already completed)"
fi

echo "All tasks completed for order_1_hide!"
