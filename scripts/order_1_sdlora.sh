...
###################!!!!!!!! bash scripts/order_1_sdlora.sh > exp/sdlora/order_1/logs/train_and_infer_sdlora.log 2>&1 &
...
CUDA_VISIBLE_DEVICES=0,1,2,3 deepspeed --master_port $port src/run_uie_lora.py \
   ...
   --output_dir exp/sdlora/order_1/outputs/1-dbpedia \
...
CUDA_VISIBLE_DEVICES=0,1,2,3 deepspeed --master_port $port src/run_uie_lora.py \
   ...
   --instruction_file configs/instruction_config.json \
   --instruction_strategy single \
   --output_dir exp/sdlora/order_1/outputs/2-amazon \
   --per_device_train_batch_size 16 \
   --per_device_eval_batch_size 128 \
   --gradient_accumulation_steps 1 \
   --learning_rate 1e-3 \
...
CUDA_VISIBLE_DEVICES=0,1,2,3 deepspeed --master_port $port src/run_uie_lora.py \
   ...
   --instruction_file configs/instruction_config.json \
   --instruction_strategy single \
   --output_dir exp/sdlora/order_1/outputs/3-yahoo \
   --per_device_train_batch_size 16 \
   --per_device_eval_batch_size 128 \
   --gradient_accumulation_steps 1 \
   --learning_rate 1e-3 \
...
CUDA_VISIBLE_DEVICES=0,1,2,3 deepspeed --master_port $port src/run_uie_lora.py \
   ...
   --do_predict \
   --predict_with_generate \
   --model_name_or_path exp/sdlora/order_1/outputs/3-yahoo/adapter \
   --data_dir CL_Benchmark \
   --task_config_dir configs/order1_configs/agnews \
   --instruction_file configs/instruction_config.json \
   --instruction_strategy single \
   --output_dir exp/sdlora/order_1/outputs/4-agnews \
   --per_device_train_batch_size 16 \
   --per_device_eval_batch_size 128 \
   --gradient_accumulation_steps 1 \
   --learning_rate 1e-3 \
